...
class clusterHits():
     ...
     def checkIfPresent(self, cassette1ID):
         
         presentCassettes = np.unique(self.hits.Cassette[~np.isnan(self.hits.Cassette)])
         
         if not(cassette1ID in presentCassettes):
             
             flag = False 
             
             print('\n \t \033[1;33mWARNING: Cassette ID ',str(cassette1ID),' not found! Skipped! This DATA file only contains Cassettes IDs:', end=' ')
     ...
     def clusterize1cassette(self, cassette1ID, timeWindow):
         ...
         present = self.checkIfPresent(cassette1ID)
     ...
     def checkIfRepeatedIDs(self,cassettesIDs):
     ...
     def clusterizeManyCassettes(self, cassettesIDs, timeWindow):
             ...
             self.checkIfRepeatedIDs(cassettesIDs)
