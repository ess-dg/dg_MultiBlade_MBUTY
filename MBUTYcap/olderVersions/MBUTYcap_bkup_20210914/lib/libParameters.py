...
class dataReduction():
    def __init__(self, cassettes, configJsonFile):
          ...
          # 0 = OFF, 1 = File With Threhsolds Loaded, 2 = User defines the Thresholds in an array sth 

          self.softThreshold = 0
          
          self.sth = np.ones((len(cassettes.cassettes),configJsonFile.numOfWires+configJsonFile.numOfStrips))  
          
    def update(self, cassettes, configJsonFile):      
...
class parameters():
    ...
    def update(self):
        ...
        self.dataReduction.update(self.cassettes,self.configJsonFile)
        self.plotting.calculateDerivedParam()
    ...
    def HistNotification(self):
        ...
        if self.plotting.hitogOutBounds is True:
            print('\t histogram outBounds param set as True (Events out of bounds stored in first and last bin)')
        else:
            print('\t histogram outBounds param set as False (Events out of bounds not stored in any bin)')
