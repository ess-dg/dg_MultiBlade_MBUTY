...
class MBUTYmain():
    ...
    def __init__(self,parameters, runFromGui=False):
        ...
        elif self.parameters.acqMode == 'pcap-sync':
        ...
        for cont, fileName in enumerate(fileDialogue.fileName):
            ...
            if self.parameters.acqMode == 'pcap-sync' or self.parameters.acqMode == 'pcap-local' or self.parameters.acqMode == 'pcap-local-overwrite' or self.parameters.acqMode == 'off':
                ...
                testing = False 
                pcap = kaf.kafka_reader(self.parameters.clockTicks.NSperClockTick, nOfPackets = self.parameters.kafkaSettings.numOfPackets, \
        ...
        if self.parameters.plotting.bareReadoutsCalculation is False:
            ...
            if self.parameters.dataReduction.calibrateVMM_ADC_ONOFF is True:
                self.calib = cal.read_json_calib(self.parameters.fileManagement.calibFilePath+self.parameters.fileManagement.calibFileName,self.parameters.config)
            ...
            if self.parameters.MONitor.MONOnOff is True:
                MON = maps.mapMonitor(self.readouts, self.parameters.config)
                if MON.flagMONfound is True:
                    self.hitsMON = MON.hits
                    
                    MONe = clu.hitsMON2events(self.hitsMON)
                    self.eventsMON = MONe.events
                    ...
                    print('\033[1;32m\t MON events: {}\033[1;37m'.format(len(self.eventsMON.timeStamp)))
            ...
            if self.parameters.fileManagement.saveReducedFileONOFF is True: 
                ...
                if (self.parameters.MONitor.MONOnOff is True) and (MON.flagMONfound is True):
        ...
        ###############################################################################
        ### plot
        self.parameters.HistNotification()
        ...
        for loop in range(numOfLoops):
            ...
            if (self.parameters.plotting.plotRawReadouts  or  self.parameters.plotting.plotReadoutsTimeStamps or self.parameters.plotting.plotChopperResets or self.parameters.plotting.plotADCvsCh) is True:
                plread = plo.plottingReadouts(self.readouts, self.parameters.config)
            ...
            if self.parameters.plotting.bareReadoutsCalculation is False:
                ### XY and XToF
                plev = plo.plottingEvents(self.events,self.allAxis,self.parameters.plotting.coincidenceWS_ONOFF)
                ...
                ############
                # MON plots
                if self.parameters.MONitor.MONOnOff is True and self.parameters.MONitor.plotMONtofPHS is True and MON.flagMONfound is True:
                    
                    plMON = plo.plottingMON(self.eventsMON,self.allAxis)
