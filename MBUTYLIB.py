...
def HISTXYToF (XX,A,YY,B,ToFx,C,coincidence,showStats):
    ...
    for k in range(0,Nev,1):
            ...
            tt =  np.int(round(((binT-1)*((C[k]-Tmin)/(Tmax-Tmin)))))
            ...
            if ( (xx >= 0) and (xx <= binX-1) ):
                ...
                # 2D hist X-ToF
                if coincidence == 1:
                     if ( (yy >= 0) and (yy <= binY-1) and (tt >= 0) and (tt <= binT-1) ):
                         XT[xx,tt]  += 1
                         count[2,0] += 1
                     else:
                         count[2,1] += 1
                elif coincidence == 0:
                     if ( (tt >= 0) and (tt <= binT-1) ):
                         XT[xx,tt]  += 1
                         count[2,0] += 1
                     else:
                         count[2,1] += 1
